...
namespace LSW {
	namespace v5 {
		namespace Handling {
			...
			__pre_load_auto::__pre_load_auto()
			{
				// If any fails, there's something wrong with Allegro. Contact support.
				// It should not fail, that's why they are assert.
				assert(init_basic());
				assert(init_audio());
				assert(init_font());
				assert(init_graphics());
				assert(init_keyboard());
				assert(init_mouse());
				assert(init_joypad());
				assert(init_touch());
			}
