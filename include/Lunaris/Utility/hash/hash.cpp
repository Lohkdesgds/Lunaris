...
namespace Lunaris {
    ...
    const unsigned int _SHA256::sha256_k[64] = //UL = uint32
    ...
    void _SHA256::update(const unsigned char* message, unsigned int len)
    {
    ...
    void _SHA256::final(unsigned char* digest)
    {
        unsigned int block_nb;
        unsigned int pm_len;
        unsigned int len_b;
    ...
    std::string sha256_f(const std::string& fpath)
    {
        FILE* fp = nullptr;
